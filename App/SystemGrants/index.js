...
exports.GenerateDiffs = (LeftReview, RightReview) => {
    ...
    const LeftFragment = LeftReview.find(fragment => fragment.fragment_name === 'System Grants');
    const LeftItems = LeftFragment.review_data.SystemGrants;

    const RightFragment = RightReview.find(fragment => fragment.fragment_name === 'System Grants');
    const RightItems = RightFragment.review_data.SystemGrants;
    ...
    cleanDiffs.sort((v1, v2) => {
        if (v1.DiffLeftVal < v2.DiffLeftVal) {
            return -1;
        };
        if (v1.DiffLeftVal > v2.DiffLeftVal) {
            return 1;
